...
pop_size = 2000       # nb of individuals (of 2*nb_chrsm chromosomes each)
...
class Individual:
    def __init__(self, id, child=None):
        ...
        #if (recomb_counter + sum(nb_recombs) + sum(nb_recombs2)) >= len(recomb_pos_random):
        recomb_pos_random = np.random.randint(1,
                        chrsm_len,
                        2*expected_nb_recomb)
        recomb_counter = 0
        ...
        start = recomb_counter
        start2 = start + sum(nb_recombs)
        # Positions de recombinaisons
        self.recomb_par1, self.recomb_par2 = [], []
        for i in range(nb_chrsm):
            #self.recomb_par1 += [recomb_pos_random[start: start+nb_recombs[i]+1]]
            #start += nb_recombs[i]+1
            #self.recomb_par2 += [recomb_pos_random[start2: start2+nb_recombs2[i]+1]]
            #start2 += nb_recombs2[i]+1
            self.recomb_par1 += [recomb_pos_random[start: start+nb_recombs[i]]]
            start += nb_recombs[i]
            self.recomb_par2 += [recomb_pos_random[start2: start2+nb_recombs2[i]]]
            start2 += nb_recombs2[i]
    ...
    def generate_from_child_population(self, child_pop, to_check):
        ...
        for id, indiv in child_pop.individuals.items():
            ...
            if par1_id not in self.individuals:
                self.individuals[par1_id] = Individual(par1_id, indiv)
            elif to_check:
                self.individuals[par1_id].final_desc[0:0] = indiv.final_desc

            if par2_id not in self.individuals:
                self.individuals[par2_id] = Individual(par2_id, indiv)
            elif to_check:
...
#            print("min", min_nb_desc, "max", max_nb_desc)
    ...
    def check_fused_segments(self):
        ...
        if genetic_mat < chrsm_len:
            print(self.back_time, "genetic mat is ", genetic_mat, "while chrsm len", chrsm_len)
...
for i in range(pop_size):
    pop.individuals[i] = Individual(i, None)
    # We initialize the whole population anyway to follow the genealogy and get the
    # genealogical ancestors
lin = Lineage(pop)
